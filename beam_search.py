...
BeamSearchState = namedtuple("BeamSearchState", ["log_probs", "finished", "lengths"])
BeamSearchStepOutput = namedtuple("BeamSearchStepOutput", ["scores", "predicted_ids", "beam_parent_ids"])
...
def create_initial_beam_state(beam_width):
  ...
  return BeamSearchState(
      ...
      finished=tf.zeros(
          [beam_width], dtype=tf.bool),
      lengths=tf.zeros(
          [beam_width], dtype=tf.int32))
...
def choose_top_k(scores_flat, beam_width):
  ...
  """
  next_beam_scores, word_indices = tf.nn.top_k(scores_flat, k=beam_width)
...
def beam_search_step(time_, logits, beam_state, params):
  ...
  probs = mask_probs(probs, eos_token, previously_finished)
  ...
  # finished previously
  lengths_to_add = tf.one_hot([eos_token] * beam_width,
                               vocab_size, 0, 1)
  ...
  next_beam_scores, word_indices = choose_top_k(scores_flat, beam_width)
  ...
  next_finished = tf.logical_or(
      ...
      tf.equal(next_word_ids, eos_token))

  # Calculate the length of the next predictions.
  # 1. Finished beams remain unchanged
  # 2. Beams that are now finished (EOS predicted) remain unchanged
  # 3. Beams that are not yet finished have their length increased by 1
  lengths_to_add = tf.to_int32(tf.not_equal(next_word_ids, eos_token))
